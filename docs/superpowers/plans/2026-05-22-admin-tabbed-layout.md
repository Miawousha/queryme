# Tabbed Full-Width Admin Dashboard — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild the `/admin` dashboard as a full-width, full-height app shell (matching the chat/KB aesthetic) with Interviewers / Conversations / Questions as tabs.

**Architecture:** `components/admin/admin-dashboard.tsx` becomes a client component with `useState` tab state. It renders a `GridBackground` + slim chrome header + an `h-11` tab band + a single scroll region for the active tab. The existing row/card components (`InterviewerCard`, `ConversationRow`, `QuestionRow`) and helpers are reused; `Stat` and `Section` are removed. No data-layer change.

**Tech Stack:** Next.js 15 (App Router), React, TypeScript, Tailwind v4.

---

## File structure

**Modified (only file):** `components/admin/admin-dashboard.tsx` — full rewrite. Stays the
single component file for the dashboard, as today.

`app/admin/page.tsx` is **unchanged**: it remains a server component (auth +
`loadAdminData`) and renders `<AdminDashboard data={data} />`. A server component
rendering a client component with a serializable `data` prop is valid.

This is one cohesive single-file UI rewrite — intermediate states would not
compile — so it is a single implementation task with the complete new file
content provided, followed by verification.

---

## Task 1: Rewrite the admin dashboard as a tabbed full-width shell

**Files:**
- Modify: `components/admin/admin-dashboard.tsx` (full rewrite)

**Context for the implementer:**
- This is a presentational component with no unit tests; the codebase has none
  for it. Verification is `pnpm typecheck` + `pnpm build`, not TDD.
- Reused components and their signatures (all already in the repo, all client-safe):
  - `GridBackground` from `@/components/grid-background` — no props.
  - `MatriceLogo` from `@/components/matrice-logo` — `{ size?: number; animated?: boolean }`.
  - `ThemeToggle` from `@/components/theme-toggle` — `{ label: string }`.
  - `LogoutButton` from `@/components/admin/logout-button` — no props.
  - `cn` from `@/lib/utils` — classname joiner.
- `AdminData` / `AdminStats` / `CONVERSATION_LIMIT` come from `@/lib/admin/data`.
  `AdminData` = `{ stats: AdminStats; conversations: Conversation[]; questions:
  QuestionForAlex[]; interviewers: Conversation[] }`. `AdminStats` =
  `{ conversations, chat, mcp, questions, unanswered, identified }` (all numbers).

- [ ] **Step 1: Replace the entire contents of `components/admin/admin-dashboard.tsx`**
...
Run: `pnpm typecheck`
Expected: PASS — no errors. (`app/admin/page.tsx` is unchanged and still passes
`data` to `AdminDashboard`.)

- [ ] **Step 3: Build**

Run: `pnpm build`
Expected: PASS — all routes compile, `/admin` included.

- [ ] **Step 4: Commit**

```bash
git add components/admin/admin-dashboard.tsx
git commit -m "feat(admin): tabbed full-width dashboard"
```

---

## Verification (controller, after Task 1)

In the running dev preview, log into `/admin` and confirm:
- Full-width, full-height layout with the animated grid background and the slim
  chrome header (logo + "Admin" + theme toggle + logout).
- Three tabs with counts; clicking switches panels; the active tab shows its
  accent underline and its secondary metric on the right of the band.
- Clicking an interviewer card switches to the Conversations tab and scrolls to
  / opens that conversation.
- Theme toggle works; no console errors.

---

## Self-review notes

- **Spec coverage:** layout (§Layout) → Step 1 shell; tabs + counts + secondary
  metric (§Tabs) → `tabs` array + `TabMeta`; client component (§Components) →
  `"use client"` + `useState`; reused components, `Stat`/`Section` removed
  (§Components) → Step 1; cross-link (§Interviewer → conversation cross-link) →
  `openConversation` + `InterviewerCard` `onOpen`; testing (§Testing) → Steps
  2-3 + controller verification.
- **No placeholders.**
- **Type consistency:** `TabId`, `AdminData`, `AdminStats`, `Conversation`,
  `QuestionForAlex`, `InterviewerIdentity`, `openConversation` /
  `onOpenConversation` / `onOpen` signatures all consistent across the file.
